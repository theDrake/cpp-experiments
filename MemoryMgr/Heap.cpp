...
void Heap::PrintInfo(int indentLevel) const {
  size_t totalBytes = 0, totalPeakBytes = 0;
  int totalInstances = 0, spacing = 20 - indentLevel * 2;
...
void Heap::GetTreeStats(size_t & totalBytes,
    size_t & totalPeak,
...
int Heap::ReportMemoryLeaks(int nBookmark1, int nBookmark2) {
  ...
  while (pHeader != NULL) {
    if (pHeader->nAllocNum >= nBookmark1 && pHeader->nAllocNum < nBookmark2 ) {
